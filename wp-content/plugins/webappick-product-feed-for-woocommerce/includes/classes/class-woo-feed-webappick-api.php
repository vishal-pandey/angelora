	...
	final class WooFeedWebAppickAPI {
		...
		public function woo_feed_review_notice() {
			...
			if ( $show_notice ) {
				$has_notice = true;
			?>
				<div class="woo-feed-notice notice notice-info is-dismissible" style="line-height:1.5;" data-which="rating" data-nonce="<?php echo esc_attr( $nonce ); ?>">
					<p><?php
						printf(
    ...
    (function($){
		...
		public function woo_feed_hide_notice() {
			...
			if ( isset( $_REQUEST['which'] ) && ! empty( $_REQUEST['which'] ) && in_array( $_REQUEST['which'], $notices ) ) {
                ...
                $user_id = get_current_user_id();
                $updated_user_meta = add_user_meta( $user_id, 'woo_feed_review_notice_dismissed', true, true );
                update_option( 'woo_feed_review_notice_next_show_time', time() + ( DAY_IN_SECONDS * 30 ) );
